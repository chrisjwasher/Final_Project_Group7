...
def augment_audio(y, sr):
    ...
    """
    if np.random.rand() < 0.5:  # Randomly apply time stretch
        y = librosa.effects.time_stretch(y, rate=np.random.uniform(0.8, 1.2))
    if np.random.rand() < 0.5:  # Randomly apply pitch shift
        y = librosa.effects.pitch_shift(y, sr=sr,
                                        n_steps=np.random.randint(-3, 3))  # Specify `sr` as a keyword argument
    if np.random.rand() < 0.5:  # Randomly add noise
        noise = 0.005 * np.random.randn(len(y))
        y = y + noise
    if np.random.rand() < 0.5:  # Randomly time shift
